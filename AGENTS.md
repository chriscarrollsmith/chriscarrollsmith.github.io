...
The dev environment is refreshed automatically by the update script (`npm install`
plus a Playwright Chromium refresh); Node, npm, and the Playwright browsers are
already present from the VM snapshot, so no manual install steps are needed.

Services and how to run them:

- Dev server (main app): `npm run dev` serves the Astro + React site on
  `http://localhost:4321`. This is all you need for interactive/UI work and hot reload.
- Production preview: `npm run build` then `npm run preview` (also on port 4321).
...
- Tests run against the production `preview` build, not the dev server. Because
  ...
  with `CI=1` (e.g. `CI=1 npm run test:e2e`) or ensure port 4321 is free first so
  Playwright starts its own `npm run preview` server.
- `npm run test:e2e` requires a prior successful `npm run build` (the preview server
  serves `dist/`).
